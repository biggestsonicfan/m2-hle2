...
| UI | Dear ImGui via **dear_bindings** (cimgui generated at build time from the `vendor/imgui` submodule, imgui 1.92.6) |
| Graphics / windowing | **Sokol** (sokol_app, sokol_gfx, sokol_imgui, sokol_glue); the handheld build (`-DM2HLE_FRONTEND=sdl3`) is SDL3 + GLES 3 with no ImGui |
...
- Memory is dispatched through a `mem_region_t` table (36 named regions on Model 2, `MEM_REGIONS_MAX` 64, MMIO via R/W callbacks).
- HLE hooks intercept by ROM address before the i960 decoder runs (256-slot table per active game profile).
- The active `game_profile_t` is resolved at ROM-load time by matching loaded file CRC32s against a registry. (now: picked by zip basename = profile `id`, `select_profile_for_zip` in `main.c`; CRC32s validate each file the profile's `load_fn` reads.)
...
- [x] Vendor third-party sources as submodules pinned to upstream commits: `sokol/`, `ImGuiFileDialog/`, `miniz/`, `imgui/`, `dear_bindings/`, `imgui_club/` (hex-grid memory editor), `noclip/` (feeds `tools/` only). cimgui is generated by dear_bindings into `<build>/cimgui-gen/` at build time (was a committed flat drop) — replaced original cimgui/cimgui submodule due to ABI mismatch causing ImGui to be unresponsive to mouse input
- [ ] `update_deps.sh` to refresh upstream deps
- [x] `CMakeLists.txt`: static libs (`cimgui`, `imguifiledialog`, `sokol`, `miniz`); per-platform link flags; `M2HLE_FRONTEND` = `sokol` / `sdl3` / `web` (Emscripten is now a full frontend, `src/main_web.c`); `M2HLE_BUILD_TESTS` builds `tests/`
...
- [x] `src/core/game_profile.h` — `game_profile_t` struct: `id` / `display_name` / `parent_zip_name`, board variant, `load_fn` / `install_fn` function pointers, `hooks[256]` + `hook_count`, `input` (see Phase 11), `quirks` (`game_quirks_t`: decoder/mesh/model-table knobs, camera addresses, `irq_handler[4]`, sound-queue addresses, `enable_68k_sound`, `board_vblank`, `geo_displaylist`, `attract_replay`, `warning_skip_addr`); profile resolved by name (CRC32 registry deferred to Phase 14; still by zip basename today)

### Phase 2 — Memory bus (board-level) ✅
- [x] `src/board/memory.h` — `memory_bus_t` with 36 named regions (incl. `TILE_MIRROR`, IRQ controller + timers from `irq_timer.h`), `mem_read8/16/32` / `mem_write8/16/32` dispatch, MMIO callbacks, `mem_init` idempotent (now: clears the six heap regions in place via `mem_region_fresh`, never frees them — see CLAUDE.md "Memory Bus"), NULL data guard on reads; IO region initialised to `0xFF`
...
- [x] Second game profile (Daytona or VF2) — deferred to Phase 14 (now: `src/profiles/fvipers.h`, Fighting Vipers, plus `src/profiles/m2snake.h`, a hook-free homebrew profile; the web build registers STF only)
...
- [x] `src/core/hle_hooks.h` — profile-driven dispatch (`g_active_profile->hooks[]`); `hle_ret()` helper that properly restores the i960 register window (mirrors `ret` instruction — no frame-depth leak); `hle_call()` and `hle_interrupt()` (an interrupt restores AC on `ret`; the run loop delivers the board IRQs through it); `g_frame_done` volatile flag
- [x] STF boot-critical hooks in `src/profiles/sfight.h` (now 9 hooks): `cop_initialize_l1` (0x0F3C) COP-ready bit, `co_processor_error_hang` (0x77F8) halt + log (replaced the old `CoProcessorErr` 0x74E4 bypass), `check_timer_4` skip, `check_timer_4_spin` timer-flag write, `interrupt_wait`/`interrupt_wait_b`/`_idle` VsyncScr injection / RAM_BASE nudge, `_700000_loop` delay skip, `variable_diff_calc` (0x11A04) frame-pace flag
...
- [x] `src/board/cop.h` — `cop_state_t` (command accumulator + 32-slot reply FIFO + pos/scale/ang/world_pos + lazy matrix + 32768-slot geo-capture ring + diagnostic snapshot); `cop_build_matrix` with left-handed Y rotation (`+sy` bottom-left) and `Ry × Rx × Rz` order; 20+ command dispatch covering STF opcodes including `0x04800909` world-pos snapshot, `0x07800F0F` bone transform reads `world_pos[]`, `0x2F005E5E` scalar-then-vector, `0x14802929` vec3×matrix
  - (now: split after the SHARC firmware sources became the reference. `cop.h` keeps only the FIFO half — arg accumulator, `COP_ARGS_STREAM` feed, `GEO_CAPTURE_SIZE` = 262144 capture ring. `sharc.h` holds state + firmware-exact math (`sharc_fw_sqrt`/`_rsqrt`/`_div`/`_atan2`, `SHARC_REPLY_MAX` 1024), `sharc_exec.h` the dispatch table and handlers, `sharc_coli.h` the fight-collision chain, `sharc_zanzou.h` the afterimage engine. Rotation is accumulated by post-multiply, not rebuilt as `Ry × Rx × Rz`; `0x07800F0F` returns the current T. See CLAUDE.md "Coprocessor".)
...
- [x] `src/board/tile_renderer.h` — composite BG/FG layers from tilemap + palette into BGR555; 16-bit byteswap on pixel bytes (XOR low bit of byte index); tilemap-entry encoding (now: tile index `entry & 0x3FFF` = `(pal_bank << 7) | char`, was `(pal_bank << 8) | (entry & 0xFF)`); FG color-0 transparency; H/V flip + scroll wrap; `composite_layers` → RGBA8; (now: four tilemaps with per-tilemap scroll and per-pair window masks, `s24_draw_tilemap`)
- [x] `src/ui/video_window.h` — CPU pixel buffer + sokol `sg_image`/`sg_view`/`sg_sampler` ownership; `video_update` composites tiles and uploads to GPU (now: back colour + BG + FG textures, composed on the CPU or, on GL backends, by a GPU shader pass; skipped on frames where nothing changed). **No ImGui window** — the original "video window with view modes" idea was dropped in favour of drawing the game directly into the swapchain, matching the stf-hle reference architecture
...
- (now: `game_render.h` also carries the textured fill pipeline that follows `model2rd.ipp`, and `geo3d.h` can render from the GEO display list the i960 builds in bufferram — `g_geo_use_list`, `game_quirks_t.geo_displaylist` — instead of reconstructing from the COP stream. `src/ui/objview*.h` is a model/object viewer and `src/ui/cop_window.h` a COP inspector.)
...
- [x] (now: the real sound board.) `sound.h` runs the game's own 68000 driver (`m68k.h` / `m68k_exec.h` / `m68k_timing.h`) against an SCSP (`scsp.h`) one sample at a time in lockstep; i960 bytes reach the SCSP MIDI buffer through the UART at 0x9C0000; `src/core/audio_out.h` drains the host ring. Graded against MAME with `tests/snd_replay.c` + `tools/mame/snd_*.py`. See CLAUDE.md "Sound board".
...
- [x] `src/board/input.h` — `input_state_t` (pending + held bitmasks), `input_keycode_to_action()` mapping sokol keycodes to abstract actions (arrows + Z/X/C/V/1/5 for P1; I/J/K/L + Del/End/PgDn/Home/2/6 for P2; F2 Service / F3 Test), `input_key_down/up`, `input_coin(bus, player)`, `input_flush(bus)` writes held into RAM and ORs momentary
  - (now: the authentic path. `input_attach` installs a read callback on the IO region (0x01C00000) that serves the active-low IN0/IN1/IN2 port bytes built from `held`; the game's own `read_sw`, run from the VsyncScr interrupt, produces held/momentary itself. `input_coin` / `input_flush` are gone. `net_held` / `use_net` carry the composed netplay mask.)
...
- [x] STF-specific `read_sw` hook (0x000017CC) — clears `0x500700` at the start of the per-frame input-read so the host UI's `input_flush` is the only authoritative source of held bits (now: removed — there is intentionally no `read_sw` hook, `sfight.h`)
- [x] `src/main.c` — `event()` routes KEY_DOWN/UP through `input_key_down/up` (gated only on key_repeat, not ImGui focus); `frame()` calls `input_flush` once per UI tick after the snapshot pull (now: no flush; the port callback reads `held` directly)

### Phase 12 — Top-level UI (partially complete)
- [x] `src/main.c` — Sokol entry (`sokol_main`), ImGui setup, window orchestration, frame loop, full menu bar (File / Profile / Emulation / Debug / Netplay)
- [ ] `debug_window.h` — aggregate debug controls / panel layout (current debug panels are individual windows wired from the menu bar) (now: `src/ui/debug_window.h` exists but is the i960 opcode unit-test window, Debug → CPU Opcode Tests; no aggregate panel)
- [x] (now, beyond the plan:) `src/main_sdl.c` handheld frontend, `src/main_web.c` browser frontend; `src/ui/mcp_bridge.h` JSON bridge (`--mcp`, `--headless`); `src/net/` RPCN netplay + `src/ui/netplay_window.h`; `src/core/av_stream.h` + `src/ui/av_capture.h` raw A/V out (`--av-port`); `src/ui/kiosk.h`; `src/ui/overlay_host.h` / `overlay_plugin.h`; `src/core/watchpoint.h`; `src/ui/m68k_window.h` / `m68k_memview.h`
...
- [ ] Linux (`X11 Xi Xcursor GL dl m` + pthreads), macOS (`-x objective-c`, QuartzCore/Cocoa/Metal), Emscripten (WebGL2, no filesystem)
  - (now: Linux x64 and Windows x64 build in CI (`.github/workflows/canary.yml`, link line adds `asound`); Emscripten is the `web` frontend (`-sUSE_WEBGL2=1`, `pages.yml`); SDL3 arm64 handheld cross-built in CI; macOS is wired in `CMakeLists.txt` (`sokol_impl.m`, `-x objective-c -fobjc-arc`, Cocoa) but not built in CI)
- [x] Clang dead-strip linker flag (Apple only)

### Phase 14 — Second-game shakeout
- [x] Boot Daytona USA or VF2 far enough to observe i960 + COP independent of STF HLE (now: done with Fighting Vipers instead, `src/profiles/fvipers.h`, on the shared board layer)
...
(now: there are both. `tests/*.c` are standalone unit/replay tests built with `M2HLE_BUILD_TESTS` (`run_tests.ps1`), and `tools/grade-*.mjs` / `match-replay.mjs` / `grade-reset.mjs` drive a `--headless --mcp` run against the explorer (`vendor/noclip`) and MAME captures — see `tools/README.md`. `grade-models.mjs` is where the polygon-decoder Jaccard below now lives.)
...
- (now, found since: the four "not" logicals invert different operands; `movl`/`movt`/`movq` honour the literal flag; `concmpo`/`concmpi` compare only when CC bit 2 is clear; an interrupt `ret` restores AC. Symptoms and details in CLAUDE.md "i960 CPU".)
...
- **`GEO_CAPTURE_SIZE` must be ≥ 32768.** It used to be 4096 and wrapped mid-frame, producing partial 3D snapshots and visible flicker. (now 262144, `constants.h`.)
- **Tile RAM mirrors at `0x01010000`**; `TILE_MIRROR` shares TILE's buffer and must precede TILE in the table.
- **`mem_init` clears heap regions in place on re-init; it never frees them** — other threads hold those pointers across a netplay reset.

### Coprocessor (COP) / geometry engine — board-level math, all games

- **Y-rotation is LEFT-HANDED**: `[ c 0 s ; 0 1 0 ; +s 0 c ]` (note `+s` in the bottom-left). Rotation order is `Ry × Rx × Rz`. (now superseded: each ang command post-multiplies the running column-major `rot[col][row]`, per the SHARC firmware; the rebuild was only right for a single clean sequence from identity. See CLAUDE.md "Coprocessor".)
- **Angles are 16-bit signed fixed-point**, `0x10000 = 360°`; only the low 16 bits are meaningful (high bits are sign-extension).
- **World-position snapshot semantics**: command `0x04800909` (set angle) snapshots current `pos[]` into `world_pos[]`. Command `0x07800F0F` (bone transform) reads `world_pos[]`, NOT current `pos[]`. Required for animated bones; if position is overwritten between angle-set and bone-transform, bones still anchor to the snapshot. (now wrong: `0x07800F0F` is `read_world_pos` and returns the current translation T[0..2] (`pos[]`, `sharc_exec.h`); `0x04800909` still fills `world_pos[]`.)
- **Use the firmware's arithmetic, not libm** — `sharc_fw_sqrt` / `_rsqrt` / `_div` / `_atan2` / `sharc_angle_word`; the low bits feed back into i960 state.
...
- **`CoProcessorErr` (STF: 0x74E4) bypass is mandatory** — boot self-test compares COP buffers; with HLE COP the test always fails. Return by jumping to `locals.rip` (saved return address from the call frame). Every Model 2 game ships some form of COP self-test — the pattern repeats, the address won't. (now: `sfight.h` has no 0x74E4 hook; it sets the COP-ready bit at `cop_initialize_l1` (0x0F3C) and halts with the error code at `co_processor_error_hang` (0x77F8) if the test still fails.)
- **Never hook `clip_point_check` / `clip_point_check_yoko`** — they write outcode bytes to `0x50E000` that `area_clip` reads.
- **Deliver interrupts with `hle_interrupt`, never `hle_call`** — `ret` from a handler must restore AC.
...
- **Sleep granularity differs by OS**: Windows `Sleep()` ≈ 1 ms; POSIX `usleep()` ≈ 1 µs. (now corrected: Windows `Sleep()` is ~15.6 ms unless something in the process has asked for a finer timer — a `--headless` run has not; the headless loop uses a high-resolution waitable timer.)
...
- The **left-handed Y-rotation** in COP is hardware behaviour, not a game choice. Confirming it against a second game would have closed the question sooner. (now: the SHARC firmware sources settled it — rotation is a post-multiplied running matrix.)
