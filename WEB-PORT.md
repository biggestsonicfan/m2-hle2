...
**Audio latency.** First heard by a person on 2026-09-19: audible, and behind the picture. Two causes, both in `audio_out.h`, both measured with `web-smoke.mjs`'s audio column. The queue target was the desktop's 8,192 frames (186 ms, sized for a bursty emu thread and a live stream), behind a 2,048-frame callback (46 ms). And a browser keeps WebAudio suspended until the first click or key, while the board fills its 16,384-frame ring: the smoke test shows it sitting at 16,383 frames (371 ms) of stale audio, which the 1% rate nudge would take ~19 s to work off. Now the host chooses (`audio_out_config_t`): the web build asks for a 1,024-frame callback and 3,072 frames queued, steered by a low-passed fill, and a ring found past three times its target is resynced in one faded skip. Measured after: **55-70 ms queued, no underruns once the game is producing sound, and one resync on the first key press** instead of seconds of drift. The desktop defaults are untouched (its resync threshold lands at 7/8 of the ring, where it was about to drop samples anyway). Not measured: the end-to-end figure, which adds the 23 ms callback and the device's own output latency.
