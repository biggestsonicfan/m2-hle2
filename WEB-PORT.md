...
The cost, stated plainly: **`requestAnimationFrame` stops in a hidden tab**, so a player who tabs away stalls their opponent, and m2-hle2 drops a session that stalls for 15 s. Mitigation: on `visibilitychange`, drive slices from a timer instead (tabs playing audio — this one is — are exempt from Chrome's background timer throttling). *As built,* it is a Worker's 8 ms timer, running from the moment a game loads: `web_background_tick` runs the owed slices whenever animation frames have gone quiet for 50 ms (a hidden tab, or a window the browser throttles), with no picture, and the page suspends its AudioContext while hidden (WEB-NETPLAY.md 5.3). If measurement at M2 says one thread is not enough, `emu_run_slice` is already the seam: `-pthread` + `coi-serviceworker` is a build flag and a file, not a redesign.
...
- **M6 — after it works.** Done: gamepads with a Controls panel (`m2hle-pad.js`, kept in `localStorage` `m2hle.pad`; a second pad plays P2); keyboard remapping in the same panel for both players (`m2hle-keys.js`, `m2hle.keys`: bindings are physical keys by `KeyboardEvent.code`, the page reads the keyboard and `main_web.c` no longer maps sokol key codes, and the bar's key hints follow player 1's bindings); touch buttons for phones, with a size / placement editor and a fullscreen button (`m2hle-touch.js`, `web-smoke.mjs --mobile --taps`); the hidden-tab timer, for every game and not only a match (a Worker's 8 ms tick into `web_background_tick`; `web-smoke.mjs --hide 12:12` holds 60 fps hidden, in headless Edge only). Open: OPFS cache; the footprint pass with numbers (wasm size, heap peak); WebTransport/WebRTC if WebSocket latency proves to be the complaint; threads only if M2's budget says so.
...
**Size** (2026-09-22, with netplay, the object viewer and the page's tools in it): `m2hle.wasm` 536 KB (201 KB gzipped), `m2hle.js` 86 KB (21 KB), page 80 KB. The footprint pass that got it there is §3.6; before it, 545 KB + 134 KB.

**Speed** (wasm under Node 24, this machine, while ~10 native emulators were also running on it): one 60 Hz slice of STF **with the sound board = 1.53 ms average, 5.5 ms worst**; the renderer's CPU side ~0.8 ms excluding tile compose (the bench's dummy backend composes tiles on the CPU; WebGL2 uses the GPU compositor). In headless Edge the game holds 60-61 fps (on the machine's real GPU: see the correction under "Tools" below). One thread is enough; section 3.3 stands.

**Where a browser frame goes, and the one thing worth giving up (2026-09-22).** Profiled in headless Edge on this machine's real GPU (`Profiler.start` over the DevTools protocol, against a build with `--profiling-funcs`, in a Sonic v Knuckles round): of the main thread's busy time, the **sound board is the largest single piece** (`sound_run`, ~20%), then the 3D draw's CPU side (~17%), then the rest of the slice (the i960 and the COP, ~14%); WebGL calls are ~20%, and of those the geometry upload is one `bufferSubData` a frame against 61 draws, so the batching is not the problem. Per frame that is **2.6 ms of board and 1.35 ms of picture** (`draw3d` 1.03 of the 1.35 — `web_perf()` now reports the five stage timers, so the page can say which stage rather than "the picture").

So there is no single slow thing to fix; but there is work that need not happen at all. requestAnimationFrame follows the **display**, so a 120 or 144 Hz screen asks for two or more pictures per board frame, and the extra ones are real work: `game_frame_lerp` interpolates the 3D between board frames. `web_afford_picture` (main_web.c) now prices that: the measured cost of a slice, of a picture, and the measured callback rate, against 850 ms of each second. Over budget, only the board's own frames are drawn and the interpolated ones are dropped; under it, nothing changes. The rule is a **sum of costs, not a symptom** deliberately — a rule that waited for the board to fall behind would switch the smoothing off and on a second at a time, because switching it off is what puts the board back on time. A 60 Hz display never reaches it: every callback there carries a new board frame, and a board frame is never what is dropped. Measured with the callback rate at 164 Hz, which is what the headless browser does and what a 144 Hz screen would ask for: **main thread 37% → 26% busy and the GPU 2.1 → 0.7 ms a frame, at the same 60.0 fps** (`?args=--pictures-board-rate` and `--pictures-all` pin either answer; the lag check reports how many were skipped). What this does NOT do is make a slow machine fast: at 60 Hz it does nothing at all.

**The page's bar is two buttons (2026-09-22).** The source link and a menu holding Play online, Fullscreen, Controls, Feels laggy? and Console. The menu's buttons keep their ids and their own handlers, so the panels and `tools/web-netplay.mjs`'s `document.getElementById('btn-online').click()` work whether it is open or not; what the menu would have shown when closed is on the button (green while a game is on, a red dot when the console has errors). With it, the bug that the bar stayed lit after Fullscreen: the bar fades on `:focus-within`, a **mouse click leaves the button focused**, and the Fullscreen button is focused on the way back out. It is `:has(:focus-visible)` now, which a click does not set and the keyboard does, `:hover` is behind `@media (hover: hover)` so a tap does not leave it lit on a phone, and leaving the bar hands the keys back to the canvas. It reproduces only under a **real** mouse event (`Input.dispatchMouseEvent`): `element.click()` does not focus anything, which is why no scripted test had caught it.
...
**The object viewer works here too** (`web/site/m2hle-objview.js`, `tools/web-objview.mjs`). The desktop build draws one model on its own, offscreen, from a camera a caller places, and writes a PNG per angle; the browser gets the same thing, because the viewer and its command vocabulary are board-level (`src/ui/objview.h`, `src/ui/objview_cmd.h`) and the readback is an FBO and `glReadPixels`, which WebGL2 has. Two things had to differ. Nothing may block -- the thread asking is the thread drawing -- so a request is armed and collected on a later animation frame, though a whole batch still renders inside one emulator frame. And there is nowhere to write: each PNG stays in the heap until the next batch and JavaScript copies it out, which is what `web-objview.mjs` turns into files over the DevTools protocol. `?objview=N` puts the model on the canvas in place of the game -- drag to orbit, wheel to zoom, Escape to go back -- with the board still running behind it. Measured: the same six angles of model 3544 came back with the same coverage and the same pixel boxes on WebGL2 as on D3D11, so the geometry path does not differ between the two.

*The trap that cost the most time here*, and it is not web-specific: "the textures are loaded" and "this model's colours are loaded" are different questions. Face colours come out of palette RAM, which the game fills **per scene**. Model 3544 at attract frame 941 drew correctly shaped, correctly textured and black-faced in the browser while the desktop at 936 drew it blue -- which looks exactly like a wasm-vs-native divergence and is not: at frame 1802 the browser draws it blue too. The two runs were simply at different points in attract. `--at-frame N` is the knob; the desktop's is `wait_frames`.

**Tools on the page, and what they found on their first run.** The tool cluster in the top-right corner has two buttons (`web/site/m2hle-tools.js`).

- *Console*: every line the emulator prints, filtered by General / Warnings / Errors (`log.h`'s own `[INFO]` `[WARN]` `[ERR ]` prefixes, and sokol's), with Copy putting what the filters show on the clipboard under a header naming the build, browser and GPU. A line repeated within 5 s folds into its first appearance as a count. Keys typed into the drawer never reach the game: sokol listens on `window` in the capture phase, so the page registers its own capture listener first and stops the event there.
- *Feels laggy?*: reads the emulator's running totals (`g_web_perf` in `main_web.c`: time in slices, time building the picture, late callbacks, board time dropped) twice, five seconds apart, plus a GPU timer query where the browser allows one, the audio queue and the renderer string, and reasons from the difference. It tells apart a CPU that cannot keep up, too few frames drawn (GPU, software renderer, a 30 fps cap), stutter from outside the emulator, sound dropouts, a slow audio device, netplay stalls and a log flood — and once one of those explains the rest, it does not list the rest as causes. When the evidence points at the GPU it offers the remedy: `web_set_render_scale` draws the game offscreen at N x 496x384 and scales up (at 2, a tenth of the pixels of a 4K canvas), live, remembered in `localStorage`. Every branch was run on synthetic measurements, and the CPU-bound one for real with DevTools' CPU throttling (`web-smoke.mjs --cpu-throttle 6 --diagnose`).
...
**Determinism: passed against MSVC on 2026-09-21.** Two fixes did it, a NaN rule and `-fno-strict-aliasing` (WEB-NETPLAY.md section 3); cross-play is on. What follows is the first measurement, against gcc, kept for the record.

**Determinism, as first measured: the gate was NOT passed, and the failure was small and specific.**
