...
- **Faces lying on faces are ranked, not left to the depth buffer** (`geo3d_mesh_layers`, from the explorer's `js/layers.js`). The board gives each polygon one depth and fills near buckets first, so of two faces in a plane one wins the whole polygon. Faces that overlap, face the same way to within ~2° and stand ≤ 0.5 apart are paired. Once per cached mesh, in model space, each face gets a layer and each group the plane of its largest face. At draw time each corner takes the depth where its view ray meets the group plane (`geo3d_zs_corner`, exact per pixel because a plane's depth is affine on screen), and the vertex shader pulls the face forward by `layer × 4` 24-bit steps.
  - *Symptom that surfaced this in STF (issue #75):* Flying Carpet's pyramid shadows (model 580: 32 checker triangles on 262 of sand, which is deeper than the recede bound) broke into shards. Casino Night's reel art (model 188, faces 25/26, 0.02 in front of the cabinet) showed orange stripes. `geo_test` holds both counts.
  - **Three departures from the explorer, each measured against MAME by `tools/grade-zsort.mjs`; do not port them back:**
    - A game frame orders each pair the way the board's sort goes from *that* camera: the nearer 12-bit key wins, and a tie goes to the later polygon (`geo3d_mesh_draw_layers`, MAME `float_to_zval`). The explorer's vote over 64 view directions is kept only for the object viewer.
    - A face sorted by its farthest corner (mode 2) keeps the recede and takes no layer or plane. Layered, Casino Night's floor emblem (model 194, base face 130) stopped stepping back and covered green that MAME shows: 3,387 pixels further from MAME.
    - Pairs held apart by more than the 0.02 tie that ask for the same corner are left to the depth buffer and join no group. The explorer's absolute 0.5 gap put a fighter's glove (1813/1818, 0.3 units across) on one plane.
  - **A group's plane may pull a face forward, never push it back** (`geo3d_zs_corner` keeps the corner's own depth where the plane is behind it), and the plane is the largest face that *takes* one, not a far-corner face. A decal paired with a far-corner wall by their different corners, 0.05–0.17 in front of it, used to go back onto the wall's plane and so behind its own far-corner screen, which keeps its depth.
    - *Symptom that surfaced this in STF (issue #85):* in the attract intro's Tails lab (model 3473), the side monitors showed flat green and blue where their pictures (faces 682/683, 300–308) should be. MAME shows the pictures. The same change took `grade-zsort --stage 5` from 20 changed pixels nearer without the layers to 7; stage 1 held.
  - With all three, the attract replay (`--match-replay-stage 1` / `5`, graded at 31 frames each) has the layers putting 1,465 of 1,469 changed pixels nearer MAME on Flying Carpet and 788 of 808 on Casino Night. `set_camera` takes `zlayers`, `zlayer_model` (`N` or `LO-HI`, for bisecting), `zlayer_board`, `zlayer_plane` and `zlayer_steps`, and reports `layer_models`. The object viewer takes `layers` and keeps only the offset, since the plane is camera space for the board's camera.
- **Closed solids standing on a deep floor opt out of the z-sort recede** (`game_quirks_t.zsort_standing`, `geo3d_model_standing`). A face sorted by its far corner (z mode 2), if it is shallower than `g_geo3d_zsort_recede`, steps back in full. A floor too deep to recede keeps its projected depth. So a solid on that floor sinks through it. The listed models keep their projected depth, which for a closed solid gives the board's answer. The list holds model numbers, so it belongs to the profile.
  - *Symptom that surfaced this in STF (issue #78):* Aurora Icefield's ice pillars (4278) had their bases cut off flat at the ice (1602), and streaks of the far wall showed through them. The ice cannot recede instead: the walrus reflection and the lower cage hang under it. The explorer made the same choice (noclip#7, `ZSORT_KEEP`). `set_camera {"zstanding":0}` turns it off for an A/B.
  - The explorer marks the walrus statues (1601) too; the list does not. The game draws them only when the camera looks across the ring at them (bit 0 of `0x500288`, a cull the explorer leaves out), and from there their feet are whole with or without the exception. The only difference is the smaller walrus's tusks, not yet checked against MAME. Add a model here only on a picture the board confirms.
- **A far-corner face lying a little in front of a far-corner face that is too deep to recede keeps its own depth** (`geo3d_mesh_keep_depth`, draw time, only when the back face's far corner is the deeper). The layer pass records these pairs (both mode 2, parallel, overlapping, more than the 0.02 tie and at most 0.5 apart), which it otherwise leaves to the depth buffer. That is right only while both keep their depth or both recede. The shallow front face recedes in full and sinks through the deep one near its own near edge. On the board the deep face sorts by its far corner, far off, so the front one wins. Unlike the standing list, this needs no model numbers. `set_camera {"zkeep":0}` turns it off for an A/B (`grade-zsort --toggle zkeep`).
  - *Symptom that surfaced this in STF (issue #85):* in the attract intro's Tails lab, the CAUTION SINGLE SEATER screen (model 3473: backing 680, stripe frame 679/681, 0.14 in front of the wall, face 6) had the wall showing through it at a grazing angle. Over the whole attract loop it changes only that monitor's frames, all of them nearer MAME; on the stage 1 and 5 replays it changes no pixel.
...
- **VS mode can carry one cold boot across matches** (`room_vs_continues`). The owner's VS setting is in the room state (`vs_mode`, like `region`), and so is `session`: the match whose cold boot the boards are running. When a VS match is decided and the two fighters are the room's only players, the owner publishes `match + 1` with `session` unchanged. Each board moves `match_started` on by one when its own board reaches the result (`netplay_end_frame`), never when the room says so, because a watcher can be a whole match behind and would file the wrong result. With anyone waiting in line the owner closes the session instead, and a board that played on past the result stops there (`netplay_member_pump`). The lockstep generation is the session's and does not change across a rematch.
  - **A VS rematch asks first, over the game** (`PS3UI_SCR_AGAIN` in `ps3ui_app.h`; the handheld's `pad_lobby.h` does the same). Without it the boards go back to character select forever and nothing on screen lets a player out. `netplay_status_t.vs_results` counts the results the board has played on past. On each one a fighter gets the PS3's result window with Play again / Exit and a 10 s timer. Timing out means stay; Exit leaves the room, which ends the session on both boards (the owner calls it off), and the one left behind is told so. It is UI only: the board runs on underneath and gets none of the pad while asked, so no protocol change.
