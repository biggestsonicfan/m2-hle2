...
- **Faces lying on faces are ranked, not left to the depth buffer** (`geo3d_mesh_layers`, a port of the explorer's `js/layers.js`). The board gives each polygon one depth and fills near buckets first, so of two faces in a plane one wins the whole polygon. Faces that overlap, face the same way to within ~2° and stand ≤ 0.5 apart are ordered by the board's sort taken from 64 view directions. Each gets a layer, and the group shares its largest face's plane. The ranking is once per cached mesh, in model space. At draw time each corner takes the depth where its view ray meets the group plane (`geo3d_zs_corner`, exact per pixel because a plane's depth is affine on screen), and the vertex shader pulls the face forward by `layer × 4` 24-bit steps. Layered faces skip the recede.
  - *Symptom that surfaced this in STF (issue #75):* Flying Carpet's pyramid shadows (model 580: 32 checker triangles on 262 of sand, which is deeper than the recede bound) broke into shards. Casino Night's reel art (model 188, faces 25/26, 0.02 in front of the cabinet) showed orange stripes. `geo_test` holds both counts. `set_camera {"zlayers":0}` turns it off for an A/B, and the object viewer takes `layers` too. It keeps only the layer offset, because the plane is camera space for the board's camera.
